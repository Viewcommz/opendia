...
const MCP_SERVER_URL = "https://opendia-mcp.gling.co.kr";
let mcpSocket = null;
...
async function getHistory(params) {
	const { keywords = "", start_date, end_date, domains = [], min_visit_count = 1, max_results = 50, sort_by = "visit_time", sort_order = "desc" } = params;
		...
		return {
			...
			metadata: {
				...
				search_params: {
					...
					date_range:
						start_date && end_date ? `${start_date} to ${end_date}` : start_date ? `from ${start_date}` : end_date ? `until ${end_date}` : null,
...
async function getSelectedText(params) {
	const { include_metadata = true, max_length = 10000 } = params;
